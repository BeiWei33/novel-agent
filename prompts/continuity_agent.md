...
2. 检查 `novel_bible`、`chapter_draft`、`characters`、`world_setting`、`relevant_facts` 之间的主角姓名、关键金额、债务、罚款、押金、合同状态、合作关系和敌对状态是否一致。
3. 检查是否遗漏重要伏笔或新增事实。
4. 提取本章产生的新事实和人物状态变化。
5. 给出可执行的修复建议。
...
- 主角姓名不一致、关键金额不一致、合作/敌对状态前后矛盾，应记为 `severity = high`、`type = fact` 或 `character`；除非正文明确解释为别名、误报、谎言或阶段变化。
- 商业文中的罚款、欠薪、押金、订单金额、合同状态、合作谈妥/未谈妥是核心连续性事实，不能只按普通背景事实处理。
