...
- `changes` 必须说明实际改了什么，不要只写“优化文风”。
- `preserved_facts` 至少覆盖本章关键事件、人物状态变化和章尾钩子。
- `style_notes` 写给后续章节作者看，聚焦可复用的表达约束，例如对白口吻、节奏密度、平台语感。
- 除非原文严重注水，不要把正文压缩到原长度的 70% 以下；番茄向优先压解释、保冲突，起点向优先保规则清晰度。
