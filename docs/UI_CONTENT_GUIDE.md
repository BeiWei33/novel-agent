...
v0.3 的质量视图目标是让作者不用读完整 JSON，也能回答四个问题：

- 本章现在能不能展示或继续写。
- 如果不能，最优先改什么。
- 是否存在跨产物一致性硬伤。
- 本次内容来自哪个 provider、model、prompt_bundle，链路是否 fallback 或 parse_error。

### 5.1 顶部质量条

推荐把以下状态放在章节编辑器或审稿面板顶部：

| 状态 | 数据源 | 展示建议 |
| --- | --- | --- |
| 发布判断 | `ReviewReport.passed` / `total_score` | `可展示` / `需返工` / `未审稿` |
| 通过线 | `ReviewReport.scores` | 显示总分、节奏、连续性、章尾钩子是否达标 |
| 一致性硬门 | `ContinuityReport.issues` + `ReviewReport.issues` | 有姓名、金额、合作状态冲突时显示 `一致性阻塞` |
| 链路健康 | `AgentRun.status` / `parse_error` | 有 fallback 或 parse_error 时显示 `模型链路异常` |
| 来源 | AgentRun provider/model/reasoning + prompt_bundle | 显示为小字，帮助区分 mock、smoke、真实模型候选 |

### 5.2 一致性硬门卡片

跨产物一致性问题优先级高于普通文风建议。前端可以把以下问题放在 ReviewReport 问题列表上方：

| 检查项 | 触发条件 | 文案示例 |
| --- | --- | --- |
| 主角姓名 | Bible / outline / draft / facts 出现两个姓名 | `主角姓名不一致，请统一为林野或确认别名设定。` |
| 关键金额 | 罚款、欠薪、押金、订单、合同金额前后不一致 | `罚款金额存在两个口径，请统一后再进入展示。` |
| 合作状态 | 一处写已谈妥，另一处写疑似谈妥或未接触 | `合作状态不一致，会影响后续章节上下文。` |
| 关系状态 | 敌对、合作、背叛、盟友状态无解释跳变 | `人物关系状态跳变，需要补过渡或修正事实。` |

展示规则：

- `severity = high` 且 `dimension/type = continuity/fact/character` 时，默认归入一致性硬门。
- 出现一致性硬门时，不建议显示“可展示”，即使总分达到 75。
- 卡片必须给作者下一步动作：统一姓名、统一金额、修正摘要/facts、补一句剧情解释。
...
| 分组 | 来源 | 用法 |
| --- | --- | --- |
| 先修硬伤 | 一致性硬门、parse_error、fallback | 阻塞展示，优先级最高 |
| 再修连载动力 | pacing / payoff / cliffhanger 问题 | 决定是否值得追读 |
| 最后精修语言 | dialogue / style / low severity issues | 人工润色阶段处理 |
...
| 来源 | 标识 | 说明 |
| --- | --- | --- |
| mock 内容 | `mock` | 来自 `docs/WEB_DEMO_CONTENT.md` 的展示包 |
| smoke provider | `smoke` | 真实 REST API + 本地确定性 provider |
| real-model candidate | provider / model | 真实模型候选样本，必须标注 prompt_bundle 和人工评分 |

`gpt-5.5 xhigh` r3 6 章样本可作为 v0.3 真实模型候选，但进入展示前应修正旧名残留、权限说明和行业解释过长问题。
