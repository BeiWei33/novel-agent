...
- 支持 `reasoning_effort` 透传到 OpenAI-compatible provider，已验证 `gpt-5.5 + xhigh` 2 章快速无重试真实链路；6 章链路已分段通过，仍需继续复跑一次性 demo 稳定性。
...
- AgentRun summary 已包含状态、耗时和 token 汇总，便于运行面板展示。
